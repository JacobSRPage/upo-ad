...
class newtonBase:
  def __init__(
    ...
    self.current_tfm = None # will repeatedly rebuild the forward map
    self.Re = Re
...
class upoSolver(newtonBase):
  ...
  def _compute_du_dt(
    ...
    """ First order time derivative estimate -- update reqd """
...
class rpoSolver(upoSolver):
  ...
  def _timestep_A(
    ...
    # TODO replace finite difference jacobian estimate with jac
    eps_new = self.eps_fd * self._jnp_norm(self.u_guess) / la.norm(eta_w_T[:self.Ndof])
